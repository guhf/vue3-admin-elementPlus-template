<template>
  <ConstUpload ref="uploadRef" class="image-uploader" v-model="state.fileList" :url="url" :limit="limit" :file-types="['png', 'jpg', 'jpeg']" 
    list-type="picture-card" :on-preview="handleImagePreview" @files-success="uploadAllSuccess">
...
import ConstUpload from './index.vue'
...
watch(() => props.modelValue, () => {
  if (props.limit === 1) {
    ...
    state.fileList = Array(props.modelValue).map(item => { return { ...Object(item), url: import.meta.env.VITE_APP_BASE_API + Object(item).fileName } })
  }
})
