    ...
    >
      <template v-for="(item,index) in calcMenuList">
        ...
        >
          ...
          >
            <span v-show="hasSubMenu && item.children" class="__menu__item-arrow-after" />
          </span>
          <div
            v-show="hoverFlag"
          ...
          >
            <template v-for="(subItem,subIndex) in item.children">
              ...
              <div v-if="subItem.line" :key="subIndex" class="__menu__line" />
            </template>
          </div>
        </div>
        <div v-if="item.line" :key="index" class="__menu__line" />
      </template>
    </div>
  </teleport>
</template>
  ...
  setup(props) {
    ...
    watch(showMenu, async(val) => {
      if (val) {
        ...
        const _arrowSize: RegExpMatchArray | null | undefined | number = props.menuItemCss?.arrowSize?.match(/\d+/)
        if (_arrowSize) {
