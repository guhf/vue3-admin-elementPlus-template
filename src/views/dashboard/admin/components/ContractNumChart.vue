...
import { getContractNumChart } from '~/apis/relation/contract'
...
const props = withDefaults(defineProps<Props>(), {
  ...
  const getData = () => {
    getContractNumChart().then((res: any) =>{
      initChart(res.data)
    })
  }

  const initChart = (options: any) => {
    const pieChart = init(document.getElementById('contractNumChart') as HTMLDivElement, 'macarons')
    pieChart.setOption({
      title: [{
        text: '合同数量',
        textStyle: {
